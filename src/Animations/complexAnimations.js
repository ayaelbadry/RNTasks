...
export default class ComplexAnimationDemo extends Component{
    constructor(){
        super();
        this.state = {
            items: 5,
    ...
    componentDidMount(){
        const { timing, sequence, delay, stagger, parallel } = Animated
       //using interpolate to make differnt animation with on animated value
        ...
        sequence([
            delay(400),
            timing(this.state.animatedValue,{toValue: 1, duration: 10000}),
            delay(1000),
            timing(this.state.animatedValue, {toValue: 2, duration: 10000}),
            delay(1000),
            parallel([
                timing(this.state.animatedColor, {
                    toValue: 100,
                    duration: 10000
                }),
                stagger(1000, [
                    timing(this.state.animatedValue, {toValue: 1}),
                    timing(this.state.animatedValue, {toValue: 0}),
                    timing(this.state.animatedValue, {toValue: 1})
                ])
            ])
        ]).start()
    ...
    getItems = () => {
        const interpolateColor = this.state.animatedColor.interpolate({
            inputRange: [0,50,100],
            outputRange: ['#D51D49','#E1AEBA','#D51D49']
        })
